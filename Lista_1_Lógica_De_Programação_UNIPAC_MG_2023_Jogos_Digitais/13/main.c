...
void menuPrincipal();
int verificaDoubleZero(double);
int verificaIntZero(double);
...
/*Função que impede que o usuário digite caracteres que não sejam numéricos e (-) (,)*/
int verificarEntradaNumerica(char entrada[1000]) {
    ...
    for (int i = inicio; entrada[i] != '\0'; i++) {
        if (!isdigit((unsigned char)entrada[i])) {
            if (entrada[i] == '.' && pontoDecimal == 0) {
                ...
                printf("\n\nERRO: SÃO PERMITIDOS SOMENTE CARACTERES NÚMERICOS (0..9), VÍRGULA (',') e HÍFEN ('-')!!!\n\n");
                system("pause");
                limparTela();
                return 1;
            }
